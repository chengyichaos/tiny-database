...
#     f_name=Field(data_type=FieldType.VARCHAR, keys=FieldKey.UNIQUE)     #f_name字段名-字段对象
            # f_id 字段值-字段对象
# )
    ...
    def add_field(self, field_name, field_obj, value=None):
        ...
        if len(self.__field_names) > 1:
            ...
            # 如果新增字段长度为0， 循环初始化新增字段数据，直到新增字段的数据长度与已存在字段的数据长度相等
    ...
    def search(self, fields, sort, format_type, **conditions):
        ...
        for index in match_index:
            ...
            if format_type == 'list':
                row = [self.__get_field_data(field_name, index)
...
#     # 创建表
# table = Table(
#     f_id=Field(data_type=FieldType.INT, keys=[
#                FieldKey.PRIMARY, FieldKey.INCREMENT]),
#     f_name=Field(data_type=FieldType.VARCHAR, keys=FieldKey.UNIQUE)
# )

# # 更新数据
# table.update({'f_name': '999'})
# data的类型
# 更新数据
